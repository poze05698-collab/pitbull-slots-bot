...
""",
    ...
    def listar_indicacoes(message):
        ...
        # Não dependemos de JOIN obrigatório: a indicação continua aparecendo
        # mesmo se algum dado de nome estiver ausente no cadastro.
        cursor.execute(
            """
            SELECT
                i.id,
        ...
        for item in lista:
            ...
            teclado.row(
                types.InlineKeyboardButton(
                    "✅ Aprovar",
                    callback_data=f"aprovar_indicacao:{indicacao_id}"
                ),
                types.InlineKeyboardButton(
                    "❌ Rejeitar",
                    callback_data=f"rejeitar_indicacao:{indicacao_id}"
                )
            )

            grupo = "✅ Sim" if grupo_confirmado else "❌ Não"

            bot.send_message(
                message.chat.id,
                f"""
🎁 <b>INDICAÇÃO #{indicacao_id}</b>

👤 <b>Indicador:</b>
{indicador_nome}
🆔 <code>{indicador_id}</code>

👤 <b>Indicado:</b>
{indicado_nome}
🆔 <code>{indicado_id}</code>
...
""",
    ...
    def callback_aprovar_indicacao(call):
        ...
        cursor.execute(
            """
            SELECT indicador_id, indicado_id, grupo_confirmado
            FROM indicacoes
            WHERE id=?
            """,
            (indicacao_id,)
        ...
        indicador_id, indicado_id, grupo_confirmado = dados

        # Confirma novamente no Telegram antes de liberar a recompensa.
        try:
            ...
            if membro.status in ("left", "kicked"):
                bot.answer_callback_query(
                    call.id,
                    "❌ O usuário ainda não está no grupo.",
                    show_alert=True
                )
                return
            ...
            bot.answer_callback_query(
                call.id,
                "❌ Não foi possível verificar a entrada no grupo.",
                show_alert=True
            )
            return
...
""",
    ...
    # ==========================================
    # REJEITAR INDICAÇÃO — PEDIR MOTIVO
    # ==========================================
    ...
    def callback_rejeitar_indicacao(call):
        ...
        cursor.execute(
            "SELECT id FROM indicacoes WHERE id=? AND UPPER(TRIM(COALESCE(status,'')))='PENDENTE'",
            (indicacao_id,)
        )
        ...
        if not cursor.fetchone():
            bot.answer_callback_query(
                call.id,
                "Indicação não encontrada ou já processada.",
                show_alert=True
            )
            return
...
Exemplo:
<code>Indicação não cumpriu as regras.</code>
""",
            parse_mode="HTML",
            reply_markup=types.ForceReply(
                selective=True,
                input_field_placeholder="Digite o motivo..."
            )
        )

    @bot.message_handler(
        func=lambda m: m.from_user.id in rejeicoes_aguardando_motivo
    )
    def receber_motivo_rejeicao(message):
        ...
        pendente = rejeicoes_aguardando_motivo.get(
        ...
        if not motivo:
            bot.send_message(
                message.chat.id,
                "❌ Digite um motivo válido."
            )
            return

        indicacao_id = pendente["indicacao_id"]
        ...
        try:
            bot.send_message(
                indicador_id,
                f"""
❌ <b>Sua indicação foi rejeitada.</b>
...
""",
        ...
        try:
            bot.send_message(
                indicado_id,
                f"""
❌ <b>Sua indicação foi reprovada.</b>

📝 <b>Motivo informado pelo administrador:</b>
{motivo}
""",
                parse_mode="HTML"
            )
