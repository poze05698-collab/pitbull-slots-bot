...
""",
    ...
    def listar_indicacoes(message):
        ...
        # Busca diretamente as indicações pendentes.
        # LEFT JOIN evita que uma indicação desapareça do painel
        # caso algum cadastro de usuário esteja incompleto.
        cursor.execute(
            """
            SELECT
                i.id,
            ...
            WHERE UPPER(COALESCE(i.status, '')) = 'PENDENTE'
            ORDER BY i.id
            """
        )
        ...
        for item in lista:
            ...
            indicador_nome = item[6] if item[6] else "Usuário"
            indicado_nome = item[7] if item[7] else "Usuário"
