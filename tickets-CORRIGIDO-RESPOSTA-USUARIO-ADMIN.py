...
def registrar(bot):
    ...
    def teclado_responder_usuario(ticket_id):
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton(
                "💬 Responder atendimento",
                callback_data=f"ticket_usuario_responder_{ticket_id}"
            )
        )
        return markup

    @bot.callback_query_handler(
        func=lambda c: c.data.startswith("ticket_usuario_responder_")
    )
    def responder_usuario_botao(call):
        user_id = call.from_user.id
        try:
            ticket_id = int(call.data.replace("ticket_usuario_responder_", "", 1))
        except Exception:
            bot.answer_callback_query(call.id, "Ticket inválido.", show_alert=True)
            return
        ticket = buscar_ticket(ticket_id)
        if not ticket or ticket[1] != user_id or ticket[4] == STATUS_FECHADO:
            bot.answer_callback_query(call.id, "Este atendimento não está disponível.", show_alert=True)
            return
        bot.answer_callback_query(call.id, "Envie sua mensagem abaixo.")
        enviar_mensagem(
            user_id,
            "💬 <b>Envie sua próxima mensagem:</b>\n\nNossa equipe continuará atendendo você por este mesmo ticket.",
            parse_mode="HTML"
        )
...
""",
            parse_mode="HTML",
            reply_markup=teclado_responder_usuario(ticket_id)
        )
...
""",
                parse_mode="HTML",
                reply_markup=teclado_ticket(ticket_id, usuario_id)
            )
