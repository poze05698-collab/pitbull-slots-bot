...
""",
    ...
    def confirmar_saque(call):
        ...
        if usuario_banido(user_id):
            bot.answer_callback_query(call.id, "Sua conta está bloqueada.", show_alert=True)
            return

        try:
            valor = float(call.data.split(":", 1)[1])
        except (ValueError, IndexError):
            bot.answer_callback_query(call.id, "Valor de saque inválido.", show_alert=True)
            return

        valor_minimo = valor_minimo_saque_atual()
        if valor < valor_minimo:
            bot.answer_callback_query(call.id, "Valor abaixo do mínimo.", show_alert=True)
            return

        # Duplo clique não pode criar dois saques nem reservar o mesmo saldo.
        try:
            cursor.execute("BEGIN IMMEDIATE")
            cursor.execute(
                "SELECT saldo, saque_pendente, pix, nome FROM usuarios WHERE id=?",
                (user_id,)
            )
            ...
            saldo, reservado, pix, nome = usuario
            disponivel = float(saldo or 0) - float(reservado or 0)
            if not pix:
                conn.rollback()
                bot.answer_callback_query(call.id, "Cadastre seu PIX primeiro.", show_alert=True)
                return
            if valor > disponivel:
                conn.rollback()
            ...
            cursor.execute(
                """
                INSERT INTO saques
                (usuario_id, valor, pix, status, motivo_rejeicao, admin_id, data, data_aprovacao)
                VALUES (?, ?, ?, ?, '', NULL, ?, NULL)
                """,
                (user_id, valor, pix, STATUS_PENDENTE, data_atual())
            )
            ...
            cursor.execute(
                """
                UPDATE usuarios
                SET saque_pendente = COALESCE(saque_pendente,0) + ?
                WHERE id=?
                """,
                (valor, user_id)
            )
        ...
        registrar_historico(user_id, "SAQUE", "Solicitação de saque", valor)
...
""",
            ...
            print(f"⚠️ Não foi possível notificar o admin sobre o saque #{saque_id}: {erro}")

        bot.answer_callback_query(call.id, "Saque enviado para análise.")
...
def aprovar_saque(saque_id, admin_id):
    ...
    try:
        cursor.execute("BEGIN IMMEDIATE")
        cursor.execute(
            "SELECT usuario_id, valor, status FROM saques WHERE id=?",
            (saque_id,)
        )
        ...
        # A troca de status e o débito são uma única operação atômica.
        cursor.execute(
            """
            UPDATE saques
            ...
            WHERE id=? AND status=?
            """,
            (STATUS_APROVADO, admin_id, data_atual(), saque_id, STATUS_PENDENTE)
        )
        ...
        cursor.execute(
            ...
            SET saldo=COALESCE(saldo,0)-?,
                saque_pendente=MAX(0,COALESCE(saque_pendente,0)-?)
            WHERE id=? AND COALESCE(saldo,0) >= ?
            """,
            (valor, valor, usuario_id, valor)
        )
        if cursor.rowcount != 1:
            conn.rollback()
            return False, "Saldo insuficiente para concluir este saque."
    ...
    registrar_historico(usuario_id, "SAQUE", "Saque aprovado", valor)
    registrar_movimentacao(usuario_id, "SAQUE_APROVADO", -valor, f"Saque #{saque_id} aprovado")
    criar_notificacao(
        usuario_id,
        "🎉 Saque aprovado",
...
def rejeitar_saque(saque_id, admin_id, motivo):
    ...
    try:
        ...
        cursor.execute(
            """
            UPDATE saques
            SET status=?, motivo_rejeicao=?, admin_id=?, data_aprovacao=?
            WHERE id=? AND status=?
            """,
            (STATUS_REJEITADO, motivo, admin_id, data_atual(), saque_id, STATUS_PENDENTE)
        )
        if cursor.rowcount != 1:
            conn.rollback()
            return False, "Este saque já foi processado."
        ...
        cursor.execute(
            """
            UPDATE usuarios
            SET saque_pendente=MAX(0,COALESCE(saque_pendente,0)-?)
            WHERE id=?
            """,
            (valor, usuario_id)
        )
