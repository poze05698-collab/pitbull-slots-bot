...
def _send_message_rastreado(chat_id, text, *args, **kwargs):
    ...
    try:
        if isinstance(chat_id, int) and chat_id > 0 and resultado is not None:
            # O teclado principal do Telegram precisa permanecer ativo.
            # Se a mensagem que enviou o ReplyKeyboardMarkup for apagada
            # quando o usuário toca em uma função, alguns clientes ocultam
            # o teclado. Por isso, essas mensagens não entram na limpeza.
