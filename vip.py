...
def registrar(bot):
    ...
    def receber_email_vip(message):
        ...
        msg_pix = bot.send_message(message.chat.id, texto, parse_mode="HTML", reply_markup=kb)
        from mercadopago_vip import registrar_mensagem_pix
        registrar_mensagem_pix(retorno["id"], message.chat.id, msg_pix.message_id)
