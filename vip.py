...
def enviar_area_vip(bot, chat_id):
    ...
    estrelas = max(1, int(cfg("vip_preco_stars", 100)))

    texto = (
        f"💎 <b>{nome}</b>\n\n"
        "Tenha benefícios exclusivos dentro do bot!\n\n"
        f"🎰 <b>{int(cfg('vip_giros_roleta', 2))}</b> giros de roleta por dia\n"
        f"🎫 <b>{int(cfg('vip_raspadinhas', 2))}</b> raspadinhas por dia\n"
        f"🍀 <b>{float(cfg('vip_chance_mult', 1.5)):g}x</b> mais chances em prêmios\n"
        f"⭐ <b>{float(cfg('vip_xp_mult', 2)):g}x</b> XP\n"
        f"🪙 <b>+{int(cfg('vip_coins_bonus', 0))}</b> Coins bônus\n"
        f"💎 <b>+{int(cfg('vip_gemas_bonus', 0))}</b> Gemas bônus\n"
    )
    ...
    if ativo:
        ...
        if online:
            kb.add(types.InlineKeyboardButton(
                f"🔄 Renovar por {estrelas} ⭐",
                callback_data="vip_comprar"
            ))
        kb.add(types.InlineKeyboardButton(
        ...
        texto += f"\n⏳ Duração: <b>{dias} dias</b>\n"
        if online:
            kb.add(types.InlineKeyboardButton(
                f"💎 Adquirir por {estrelas} ⭐",
                callback_data="vip_comprar"
            ))
        kb.add(types.InlineKeyboardButton(
    ...
    texto += "\n\n💡 Você também pode adquirir o VIP pelo suporte e receber um código promocional."
...
def registrar(bot):
    ...
    def vip_cfg_callback(call):
        ...
        if key == 'toggle':
            ...
            return
        if key == 'online':
            novo = '0' if cfg('vip_pagamento_online','1') == '1' else '1'
            set_cfg('vip_pagamento_online', novo)
            bot.answer_callback_query(call.id, 'Pagamento online alterado.')
            bot.delete_message(call.message.chat.id, call.message.message_id)
            mostrar_admin_vip(bot, call.message.chat.id)
            return
...
def mostrar_admin_vip(bot, chat_id):
    ...
    kb.row(
        types.InlineKeyboardButton("💰 Preço em Stars", callback_data="vip_cfg_price"),
    ...
    kb.row(
        types.InlineKeyboardButton(
            "🔴 Desativar Pagamento Online" if online else "🟢 Ativar Pagamento Online",
            callback_data="vip_cfg_online"
        )
    )
    kb.row(
    ...
    online_status = "🟢 ATIVO" if online else "🔴 DESATIVADO"

    texto = (
        "💎 <b>CONFIGURAÇÃO VIP</b>\n\n"
        f"Status: {status}\n"
        f"Preço: <b>{cfg('vip_preco_stars','100')} ⭐</b>\n"
        ...
        f"Pagamento online: <b>{online_status}</b>\n\n"
        "Se o pagamento online estiver desativado, o usuário continuará "
        "vendo a opção de aquisição pelo suporte."
    )
    bot.send_message(chat_id, texto, parse_mode="HTML", reply_markup=kb)
