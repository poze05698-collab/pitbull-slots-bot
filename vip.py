...
def enviar_area_vip(bot, chat_id):
    preparar_vip()
    b = beneficios_vip(chat_id)
    if b["ativo"]:
        cursor.execute("""SELECT expiracao FROM vip_assinaturas
                         WHERE usuario_id=? AND status='ATIVO'
                         ORDER BY expiracao DESC LIMIT 1""", (chat_id,))
        row = cursor.fetchone()
        exp = row[0] if row else "-"
        bot.send_message(chat_id, f"💎 <b>{cfg('vip_nome','VIP Premium')}</b>\n\n"
                         f"🟢 Status: <b>ATIVO</b>\n⏳ Expira: <b>{exp}</b>\n\n"
                         f"🎰 Roleta: <b>{b['roleta']} giros/dia</b>\n"
                         f"🎫 Raspadinha: <b>{b['raspadinha']} por dia</b>\n"
                         f"🍀 Chance: <b>{b['chance']}x</b>\n"
                         f"⭐ XP: <b>{b['xp']}x</b>", parse_mode='HTML')
        return
    if cfg("vip_ativo", "1") != "1":
        bot.send_message(chat_id, "💎 O VIP está temporariamente indisponível.")
        return
    stars = max(1, int(cfg("vip_preco_stars", 100)))
    dias = max(1, int(cfg("vip_duracao_dias", 30)))
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(f"💎 Comprar por {stars} ⭐", callback_data="vip_comprar"))
    bot.send_message(chat_id,
        f"💎 <b>{cfg('vip_nome','VIP Premium')}</b>\n\n"
        f"⏳ Duração: <b>{dias} dias</b>\n\n"
        f"🎰 {int(cfg('vip_giros_roleta',2))} giros de roleta/dia\n"
        f"🎫 {int(cfg('vip_raspadinhas',2))} raspadinhas/dia\n"
        f"🍀 {float(cfg('vip_chance_mult',1.5)):g}x mais chance em prêmios\n"
        f"⭐ {float(cfg('vip_xp_mult',2)):g}x XP\n\n"
        f"💰 Preço: <b>{stars} ⭐ Telegram Stars</b>", parse_mode='HTML', reply_markup=kb)
...
def registrar(bot):
    ...
    def comprar_vip(call):
        ...
        if cfg("vip_ativo", "1") != "1":
            ...
            return
        if vip_ativo(uid):
            bot.answer_callback_query(call.id, "Você já possui VIP ativo.", show_alert=True)
            return
        stars = max(1, int(cfg("vip_preco_stars", 100)))
        dias = max(1, int(cfg("vip_duracao_dias", 30)))
    ...
    def vip_pre_checkout(query):
        try:
            ...
            if not int(cfg("vip_ativo", "1")):
                ...
                return
            if vip_ativo(uid):
                bot.answer_pre_checkout_query(query.id, ok=False, error_message="Você já possui VIP ativo.")
                return
    ...
    def vip_pagamento(message):
        ...
        try:
            ...
            if cursor.fetchone():
                return
            inicio = _agora(); expiracao = inicio + timedelta(days=dias)
            ...
            cursor.execute("""INSERT INTO vip_assinaturas(usuario_id,plano,preco_stars,inicio,expiracao,status,charge_id,payload,criado_em)
                            VALUES(?,?,?,?,?,'ATIVO',?,?,?)""", (uid,cfg('vip_nome','VIP Premium'),stars,inicio.isoformat(timespec='seconds'),expiracao.isoformat(timespec='seconds'),charge,payload,data_atual()))
            conn.commit()
    ...
    def vip_cfg_callback(call):
        ...
        if key == 'toggle':
            ...
            return
        mapping = {'name':('vip_nome','Nome do VIP'), 'price':('vip_preco_stars','Preço em Stars'), 'days':('vip_duracao_dias','Duração em dias'), 'roleta':('vip_giros_roleta','Giros de roleta por dia'), 'rasp':('vip_raspadinhas','Raspadinhas por dia'), 'chance':('vip_chance_mult','Multiplicador de chance'), 'xp':('vip_xp_mult','Multiplicador de XP'), 'coins':('vip_coins_bonus','Coins bônus'), 'gemas':('vip_gemas_bonus','Gemas bônus')}
    ...
    @bot.message_handler(func=lambda m: m.text == '💎 VIP' and eh_admin(m.from_user.id))
    def vip_admin_menu(message):
...
def mostrar_admin_vip(bot, chat_id):
    preparar_vip()
    kb = types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton('🟢/🔴 Ativar VIP', callback_data='vip_cfg_toggle'), types.InlineKeyboardButton('✏️ Nome', callback_data='vip_cfg_name'))
    kb.row(types.InlineKeyboardButton('💰 Preço em Stars', callback_data='vip_cfg_price'), types.InlineKeyboardButton('⏳ Duração', callback_data='vip_cfg_days'))
    kb.row(types.InlineKeyboardButton('🎰 Giros', callback_data='vip_cfg_roleta'), types.InlineKeyboardButton('🎫 Raspadinhas', callback_data='vip_cfg_rasp'))
    kb.row(types.InlineKeyboardButton('🍀 Chances', callback_data='vip_cfg_chance'), types.InlineKeyboardButton('⭐ XP', callback_data='vip_cfg_xp'))
    kb.row(types.InlineKeyboardButton('🪙 Coins bônus', callback_data='vip_cfg_coins'), types.InlineKeyboardButton('💎 Gemas bônus', callback_data='vip_cfg_gemas'))
    status = '🟢 ATIVO' if cfg('vip_ativo','1') == '1' else '🔴 DESATIVADO'
    bot.send_message(chat_id, f"💎 <b>CONFIGURAÇÃO VIP</b>\n\nStatus: {status}\nPreço: <b>{cfg('vip_preco_stars','100')} ⭐</b>\nDuração: <b>{cfg('vip_duracao_dias','30')} dias</b>\nRoleta: <b>{cfg('vip_giros_roleta','2')}/dia</b>\nRaspadinha: <b>{cfg('vip_raspadinhas','2')}/dia</b>\nChance: <b>{cfg('vip_chance_mult','1.5')}x</b>\nXP: <b>{cfg('vip_xp_mult','2')}x</b>\nCoins bônus: <b>{cfg('vip_coins_bonus','0')}</b>\nGemas bônus: <b>{cfg('vip_gemas_bonus','0')}</b>", parse_mode='HTML', reply_markup=kb)
