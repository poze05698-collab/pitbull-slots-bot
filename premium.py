...
def preparar_premium():
    ...
    conn.commit()
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_raspadinhas_usuario_data ON raspadinhas(usuario_id, data)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_parceiros_ativo ON parceiros(ativo)")
    conn.commit()
...
def registrar(bot):
    ...
    def raspar(c):
        ...
        # Reserva a cartela dentro de uma transação. Isso impede que dois
        # cliques simultâneos ultrapassem o limite diário.
        try:
            cursor.execute("BEGIN IMMEDIATE")
            cursor.execute(
                'SELECT COUNT(*) FROM raspadinhas WHERE usuario_id=? AND data=?',
                (uid, hoje)
            )
            usados = cursor.fetchone()[0]
            if usados >= limite:
            ...
            tipo, premio = sortear_premio(
                'raspadinha_premios',
                [('NADA',0,35),('COINS',10,25),('COINS',25,15),
                 ('GEMAS',1,12),('SALDO',0.10,8),('SALDO',0.25,4),
                 ('SALDO',0.50,1)]
            )
            cursor.execute(
                'INSERT INTO raspadinhas(usuario_id,premio_tipo,premio_valor,data) VALUES(?,?,?,?)',
                (uid, tipo, premio, hoje)
            )
            conn.commit()
        except Exception:
            try:
        ...
        bot.edit_message_text(
            f'🎉 <b>RESULTADO DA RASPADINHA</b>\n\n{msg}\n\n'
            f'🎫 Cartelas restantes hoje: <b>{restantes}</b>',
            c.message.chat.id,
