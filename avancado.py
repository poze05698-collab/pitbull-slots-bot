...
from premium import carregar_premios_config, sortear_premio

VIP = [(1,0,'🥉 Normal'),(2,50,'🥈 VIP Bronze'),(3,150,'🥇 VIP Prata'),(4,350,'💎 VIP Ouro'),(5,750,'👑 VIP Elite')]
LOJA = {
...
def preparar_banco():
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_inventario_usuario ON inventario(usuario_id)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_clan_membros_usuario ON clan_membros(usuario_id)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_clan_membros_clan ON clan_membros(clan_id)")
    conn.commit()
...
def registrar(bot):
    ...
    def girar(c):
        ...
        cursor.execute(
            '''
            UPDATE economia_avancada
            SET giros_dia = CASE
                    WHEN ultimo_giro=? THEN COALESCE(giros_dia,0)+1
                    ELSE 1
                END,
                ultimo_giro=?
            WHERE usuario_id=?
              AND (COALESCE(ultimo_giro,'')<>? OR COALESCE(giros_dia,0) < ?)
            ''',
            (hoje, hoje, uid, hoje, limite)
        )
        if cursor.rowcount != 1:
            conn.rollback()
            bot.answer_callback_query(c.id, 'Você já usou todos os giros de hoje.', show_alert=True)
            return
        conn.commit()

        tipo, valor = sortear_premio(
            'roleta_premios',
            [('NADA',0,35),('COINS',10,30),('COINS',25,18),
             ('GEMAS',1,12),('GEMAS',2,4),('SALDO',0.10,1)]
        )

        if tipo == 'COINS':
            add_coins(uid, int(valor))
            texto = f'🪙 +{int(valor)} Coins'
        elif tipo == 'GEMAS':
            add_gemas(uid, int(valor))
            texto = f'💎 +{int(valor)} Gema(s)'
        elif tipo == 'SALDO':
            adicionar_saldo(uid, float(valor))
            registrar_historico(uid, 'ROLETA', 'Prêmio da roleta', float(valor))
            texto = f'💰 +{dinheiro(float(valor))}'
        else:
            texto = '😢 Não ganhou prêmio desta vez.'

        bot.answer_callback_query(c.id, '🎉 Resultado!')
        bot.send_message(uid, f'🎰 <b>RESULTADO</b>\n\n{texto}', parse_mode='HTML')
    ...
    def abrir_caixa(c):
        ...
        cursor.execute(
            "UPDATE economia_avancada SET caixas=caixas-1 WHERE usuario_id=? AND COALESCE(caixas,0)>0",
            (uid,)
        )
        if cursor.rowcount != 1:
            conn.rollback()
            bot.answer_callback_query(c.id, 'Sem caixas.', show_alert=True)
            return
        conn.commit()

        t, v = random.choice([('coins',50),('coins',100),('gemas',2),('saldo',3)])
        if t == 'coins':
            add_coins(uid, v)
            msg = f'🪙 +{v} coins'
        elif t == 'gemas':
            ...
            adicionar_saldo(uid, v)
            registrar_historico(uid, 'CAIXA', 'Prêmio de caixa surpresa', v)
            msg = f'💰 +{dinheiro(v)}'
    ...
    def compra(c):
        ...
        nome, custo, valor, tipo = LOJA[k]
        garantir(uid)

        cursor.execute(
            "UPDATE economia_avancada SET coins=coins-? WHERE usuario_id=? AND COALESCE(coins,0)>=?",
            (custo, uid, custo)
        )
        if cursor.rowcount != 1:
            conn.rollback()
            bot.answer_callback_query(c.id, 'Coins insuficientes.', show_alert=True)
            return
        conn.commit()

        if tipo == 'SALDO':
            adicionar_saldo(uid, valor)
            registrar_historico(uid, 'LOJA', f'Compra: {nome}', valor)
        elif tipo == 'GEMA':
            add_gemas(uid, 1)
        else:
            cursor.execute(
                '''
                INSERT INTO inventario(usuario_id,item,quantidade,data)
                VALUES(?,?,1,?)
                ON CONFLICT(usuario_id,item)
        ...
        bot.answer_callback_query(c.id, 'Compra realizada!')
        bot.send_message(uid, f'🛒 <b>Compra concluída!</b>\n\n{nome}', parse_mode='HTML')
