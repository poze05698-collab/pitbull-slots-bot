...
def registrar(bot):
    ...
    def girar(c):
        ...
        tipo,valor=sortear_premio(
            'roleta_premios',
            [('NADA',0,35),('COINS',10,30),('COINS',25,18),('GEMAS',1,12),('GEMAS',2,4),('SALDO',0.10,1)]
        )
        ...
        if tipo=='COINS': add_coins(uid,int(valor)); texto=f'🪙 +{int(valor)} Coins'
        elif tipo=='GEMAS': add_gemas(uid,int(valor)); texto=f'💎 +{int(valor)} Gema(s)'
        elif tipo=='SALDO': adicionar_saldo(uid,float(valor)); registrar_historico(uid,'ROLETA','Prêmio da roleta',float(valor)); texto=f'💰 +{dinheiro(float(valor))}'
