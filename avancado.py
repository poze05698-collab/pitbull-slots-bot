...
def registrar(bot):
    ...
    def vip_cmd(m):
        uid=m.from_user.id; xp,n,nome=vip(uid); prox=next((r for i,r,_ in VIP if i>n),None); falta=max(0,(prox-xp) if prox else 0)
        bot.send_message(uid,f'💎 <b>ÁREA VIP</b>\n\n{nome}\n⭐ VIP XP: <b>{xp}</b>\n🎯 Faltam: <b>{falta}</b> XP para o próximo nível.',parse_mode='HTML')
    ...
    def roleta(m):
        ...
        if giros>=1: bot.send_message(uid,'🎰 Você já usou sua roleta hoje. Volte amanhã!'); return
        markup=types.InlineKeyboardMarkup(); markup.add(types.InlineKeyboardButton('🎰 GIRAR',callback_data='av_girar'))
        bot.send_message(uid,'🎰 <b>ROLETA DIÁRIA</b>\n\nVocê tem 1 giro por dia.',parse_mode='HTML',reply_markup=markup)
    ...
    def girar(c):
        ...
        if last==hoje and g>=1: bot.answer_callback_query(c.id,'Você já girou hoje.',show_alert=True); return
        premios=[('coins',10),('coins',25),('coins',50),('gemas',1),('saldo',1),('nada',0)]
        tipo,valor=random.choice(premios)
        cursor.execute('UPDATE economia_avancada SET giros_dia=?,ultimo_giro=? WHERE usuario_id=?',(1,hoje,uid)); conn.commit()
