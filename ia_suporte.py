...
def _prompt(user_id, ticket_id, categoria, mensagem):
    return f"""Você é a atendente virtual oficial da Central de Suporte PIT BONUS BOT.
...
def _fallback(mensagem,user_id):
    ...
    # Encerramentos cordiais continuam recebendo resposta da IA.
    # O ticket só será fechado imediatamente se o usuário pedir explicitamente.
    if any(x in m for x in ("pode fechar", "pode encerrar", "encerra o chamado", "feche o chamado", "pode finalizar")):
        return "Por nada! 😊 Fico feliz em ajudar. Vou encerrar este atendimento agora. 👋", True
    if any(x in m for x in ("obrigado", "obrigada", "valeu", "valeu mesmo", "ok obrigado", "ok obrigada", "tá bom", "ta bom", "beleza", "resolvido")):
        return "Por nada! 😊 Se precisar de algo novamente, estou à disposição. 👋", False
