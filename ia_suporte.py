...
# Um lock por ticket evita que duas respostas da IA sejam geradas ao mesmo tempo
# para a mesma conversa e cheguem fora de ordem.
_locks_tickets = {}

def _lock_ticket(ticket_id):
    with _lock:
        if ticket_id not in _locks_tickets:
            _locks_tickets[ticket_id] = threading.Lock()
        return _locks_tickets[ticket_id]
...
def responder(user_id, ticket_id, categoria, mensagem):
    """Responde cada mensagem do ticket.

    Não existe cooldown por usuário: cada mensagem recebida enquanto o ticket
    estiver aberto pode gerar uma nova resposta. Um lock por ticket apenas
    serializa as respostas para preservar a ordem da conversa.
    """
    ...
    lock_ticket = _lock_ticket(ticket_id)
    with lock_ticket:
        prompt = _prompt(user_id, ticket_id, categoria, mensagem)
        resposta = _gemini(prompt)
