...
# =====================================================
# TRAVA ABSOLUTA DE MANUTENÇÃO
# =====================================================
# Além do middleware, usamos um handler de prioridade máxima. Isso garante
# que nenhuma função de usuário seja executada durante a manutenção, mesmo
# que uma atualização já esteja em processamento ou que outro middleware
# não consiga interromper o fluxo. Administradores continuam operando.

@bot.message_handler(func=lambda m: True, content_types=["text"])
def _bloqueio_manutencao_mensagem_prioritario(message):
    try:
        from manutencao import esta_em_manutencao
        from utils import eh_admin

        if not esta_em_manutencao() or eh_admin(message.from_user.id):
            return SkipHandler()

        bot.send_message(
            message.chat.id,
            "🛠️ <b>BOT EM MANUTENÇÃO</b>\n\n"
            "No momento, as funções do bot estão temporariamente bloqueadas.\n"
            "Nossa equipe está realizando uma manutenção para melhorar o sistema.\n\n"
            "⏳ Tente novamente em alguns minutos.",
            parse_mode="HTML"
        )
        return
    except Exception as erro:
        print(f"ERRO NA TRAVA PRIORITÁRIA DE MANUTENÇÃO: {erro}")
        return SkipHandler()

@bot.callback_query_handler(func=lambda call: True)
def _bloqueio_manutencao_callback_prioritario(call):
    try:
        from manutencao import esta_em_manutencao
        from utils import eh_admin

        if not esta_em_manutencao() or eh_admin(call.from_user.id):
            return SkipHandler()
        ...
        print(f"ERRO NA TRAVA PRIORITÁRIA DE MANUTENÇÃO (CALLBACK): {erro}")
        return SkipHandler()
