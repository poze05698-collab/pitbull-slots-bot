...
def start(message):
    ...
    if indicacao:
        # Estrutura: id, indicador, indicado, valor, status,
        # grupo_confirmado, admin_id, data, data_aprovacao
        status_indicacao = indicacao[4]
        grupo_confirmado = indicacao[5]

        if status_indicacao == "APROVADO":
            # A indicação já terminou. Nunca mais mostra o convite.
            mostrar_etapa_grupo = False

        elif grupo_confirmado == 1:
            # Já entrou e confirmou. Não pede para entrar novamente.
            mostrar_etapa_grupo = False
            mensagem_status = """
