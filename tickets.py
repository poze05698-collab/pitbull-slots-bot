...
def saudacao_suporte(nome):
    ...
    return (
        ...
        "👨‍💼 Nosso atendimento é realizado por administradores. "
        "Um de nossos atendentes irá analisar sua solicitação e responder neste mesmo atendimento.\n\n"
        "⏳ Por favor, aguarde a resposta de um administrador e envie todas as informações "
        "importantes sobre o problema para facilitar o atendimento."
    )
...
def registrar(bot):
    ...
    # ATENDIMENTO HUMANO
    # =====================================================
    # A Central de Suporte é 100% humana. Estas funções ficam
    # apenas por compatibilidade com versões antigas e não chamam IA.
    ...
    def _fechar_ticket_por_inatividade(ticket_id, usuario_id):
        # Atendimento humano: não fecha automaticamente por inatividade.
        return

    def monitorar_inatividade():
        # Mantido por compatibilidade; não fecha tickets humanos.
        return
    ...
    def registrar_mensagem_telegram(ticket_id, chat_id, message_id):
        try:
            cursor.execute(
                """
                INSERT OR IGNORE INTO ticket_mensagens_telegram
                (ticket_id, chat_id, message_id)
                VALUES (?, ?, ?)
                """,
                (int(ticket_id), int(chat_id), int(message_id))
            )
            conn.commit()
    ...
    def enviar_mensagem(chat_id, texto, *args, ticket_id=None, **kwargs):
        # Registra o ID de toda mensagem enviada pelo bot dentro de um ticket.
        msg = bot.send_message(chat_id, texto, *args, **kwargs)
        try:
            tid = ticket_id
            if tid is None and isinstance(chat_id, int) and chat_id > 0:
    ...
    def limpar_mensagens_ticket(ticket_id, chat_id):
        """
        Apaga somente as mensagens do atendimento no chat privado.
        O texto do histórico permanece em ticket_mensagens.
        """
        try:
            cursor.execute(
                """
                SELECT message_id
                FROM ticket_mensagens_telegram
                WHERE ticket_id=? AND chat_id=?
                ORDER BY id ASC
                """,
                (int(ticket_id), int(chat_id))
            )
            ids = [int(row[0]) for row in cursor.fetchall()]
        except Exception as erro:
            print(f"ERRO AO LOCALIZAR MENSAGENS DO TICKET #{ticket_id}: {erro}")
            ids = []

        apagadas = 0
        falhas = 0

        # Telegram permite que bots apaguem mensagens recebidas do usuário
        # em chats privados, desde que estejam dentro da janela permitida.
        # Apagamos individualmente para manter compatibilidade com a versão
        # instalada do pyTelegramBotAPI.
        for message_id in ids:
            try:
                ...
                falhas += 1
                print(
                    f"LIMPEZA TICKET #{ticket_id}: "
                    f"não foi possível apagar message_id={message_id}: {erro}"
                )

        try:
            cursor.execute(
                """
                DELETE FROM ticket_mensagens_telegram
                WHERE ticket_id=? AND chat_id=?
                """,
                (int(ticket_id), int(chat_id))
            )
            ...
            print(f"ERRO AO LIMPAR REGISTRO DAS MENSAGENS DO TICKET #{ticket_id}: {erro}")

        print(
            f"🧹 LIMPEZA TICKET #{ticket_id}: "
            f"{apagadas} apagadas, {falhas} não apagadas."
        )
