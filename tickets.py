...
def saudacao_suporte(nome):
    ...
    return (
        f"{periodo}, {nome}! 👋\n\n"
        "🤝 <b>Seja bem-vindo(a) à Central de Suporte PITBULL SLOTS.</b>\n\n"
...
def preparar_banco():
    ...
    cursor.execute("PRAGMA table_info(ticket_mensagens)")
    colunas_mensagens = {linha[1] for linha in cursor.fetchall()}
    if "telegram_message_id" not in colunas_mensagens:
        cursor.execute(
            "ALTER TABLE ticket_mensagens ADD COLUMN telegram_message_id INTEGER"
        )
...
def registrar(bot):
    ...
    def atendimento_ia_em_background(ticket_id, usuario_id, categoria, mensagem):
        ...
        try:
            from ia_suporte import responder_com_status
            resposta, encaminhar_humano = responder_com_status(usuario_id, ticket_id, categoria, mensagem)
            if not resposta:
                return
            ...
            if encaminhar_humano:
                cursor.execute(
                    """UPDATE tickets SET resposta=?, status=?, data_resposta=?, assumido_por=?
                       WHERE id=? AND status<>? AND assumido_por IS NULL""",
                    (resposta, STATUS_ABERTO, data_atual(), -1, ticket_id, STATUS_FECHADO)
                )
                conn.commit()
                enviado = bot.send_message(
                    usuario_id,
                    f"🤖 <b>Central de Suporte — Ticket #{ticket_id}</b>\n\n{resposta}\n\n"
                    "👨‍💼 Este caso precisa de uma análise da nossa equipe. Já encaminhei o chamado para um administrador.\n\n"
                    "⏳ Aguarde o atendimento humano.",
                    parse_mode="HTML"
                )
                registrar_mensagem(
                    ticket_id, usuario_id, "IA", resposta, enviado.message_id
                )
                notificar_admin_nova_mensagem(ticket_id, usuario_id, mensagem, mostrar_botoes=True)
                return

            cursor.execute(
                """UPDATE tickets SET resposta=?, status=?, data_resposta=?
                   WHERE id=? AND status<>? AND assumido_por IS NULL""",
    ...
    def _fechar_ticket_por_inatividade(ticket_id, usuario_id):
        try:
            ...
            bot.send_message(usuario_id, f"🤖 <b>Central de Suporte — Ticket #{ticket_id}</b>\n\nComo não recebemos uma nova mensagem nos últimos 5 minutos, este atendimento foi encerrado automaticamente. 👋\n\nSe precisar de ajuda novamente, abra um novo chamado.", parse_mode="HTML")
            try:
                bot.send_message(usuario_id, "⭐ <b>Gostaria de avaliar o atendimento?</b>", parse_mode="HTML", reply_markup=teclado_avaliacao(ticket_id))
    ...
    def registrar_mensagem(ticket_id, usuario_id, remetente, mensagem, telegram_message_id=None):
    ...
    def limpar_mensagens_ticket(ticket_id, usuario_id):
        """Apaga do chat as mensagens do atendimento que foram registradas.
        O histórico permanece integralmente em ticket_mensagens no banco.
        """
        try:
            cursor.execute(
                "SELECT telegram_message_id FROM ticket_mensagens "
                "WHERE ticket_id=? AND usuario_id=? AND telegram_message_id IS NOT NULL",
                (ticket_id, usuario_id),
            )
            ids = [row[0] for row in cursor.fetchall()]
        except Exception as erro:
            print(f"ERRO AO BUSCAR MENSAGENS DO TICKET #{ticket_id}: {erro}")
            return
        ...
        for message_id in ids:
            try:
                bot.delete_message(usuario_id, int(message_id))
            except Exception:
                pass
...
""",
    ...
    def receber_primeira_mensagem(message):
        ...
        abertura_enviada = bot.send_message(
            message.chat.id,
            f"""
...
""",
        ...
        registrar_mensagem(
            ticket_id, user_id, "SISTEMA",
            "Ticket aberto com sucesso.", abertura_enviada.message_id
        )

        # Saudação inicial profissional da Central de Suporte.
        try:
            nome_usuario = (message.from_user.first_name or "Usuário").strip()
            saudacao_enviada = bot.send_message(
                user_id,
                saudacao_suporte(nome_usuario),
                parse_mode="HTML"
            )
            registrar_mensagem(
                ticket_id, user_id, "SISTEMA",
                "Saudação inicial da Central de Suporte.",
                saudacao_enviada.message_id
            )
        except Exception as erro:
            print(f"ERRO AO ENVIAR SAUDAÇÃO DO SUPORTE: {erro}")

        # A IA começa imediatamente o atendimento da primeira mensagem.
    ...
    def continuar_conversa_usuario(message):
        ...
        try:
            ...
            # Se nenhum administrador assumiu o ticket, a IA responde a esta mensagem.
            if ticket[7] is None:
                iniciar_atendimento_ia(
                    ticket_id,
                    user_id,
...
""",
    ...
    def receber_resposta_admin(message):
        ...
        try:
            ...
            resposta_enviada = bot.send_message(
                usuario_id,
                f"""
...
""",
            ...
            cursor.execute(
                "UPDATE ticket_mensagens SET telegram_message_id=? "
                "WHERE id=(SELECT MAX(id) FROM ticket_mensagens WHERE ticket_id=? AND remetente='ADMIN')",
                (resposta_enviada.message_id, ticket_id),
            )
            conn.commit()
...
""",
    ...
    def fechar_ticket(call):
        ...
        # O histórico permanece no banco; o chat do usuário é limpo.
