...
def registrar(bot):
    ...
    def atendimento_ia_em_background(ticket_id, usuario_id, categoria, mensagem):
        ...
        try:
            ...
            # Pedido explícito do usuário para encerrar: salva a resposta e fecha.
            pedido_fechamento = any(x in mensagem.lower() for x in (
                "pode fechar", "pode encerrar", "encerra o chamado",
                "feche o chamado", "pode finalizar"
            ))
            if pedido_fechamento:
                cursor.execute(
                    "UPDATE tickets SET status=?, fechado_em=? WHERE id=? AND status<>? AND assumido_por IS NULL",
                    (STATUS_FECHADO, data_atual(), ticket_id, STATUS_FECHADO)
                )
                conn.commit()
                ...
                enviar_mensagem(
                    ...
                    parse_mode="HTML", ticket_id=ticket_id
                )
                return
